...
public class EvalPanel extends JPanel{
	...
	*/
	...
	public final UnaryOperator<String> io;
	
	public EvalPanel(UnaryOperator<String> io){
		io.hashCode(); //check for null
		this.io = io;
		/*
		def plus(x,y):
			return x+y
			...
			public void keyReleased(KeyEvent e){
				...
				if(s.endsWith("\n")){
					...
					if(!command.isEmpty()){
						...
						String response = "";
						try{
							Object ret = eval(command);
							response = ""+ret;
						}catch(Exception ex){
							response = ("ERR: "+ex).trim(); //TODO printStackTrace to string
						}
						textarea.setText(s+outPrefix+response+"\n");
	...
	/** FIXME if this is called externally then it wont display here */
	...
	public static final String outPrefix = "//";
	...
	*/
	...
	public static void main(String[] args){
		...
		JFrame window = new JFrame("Wikibinator - a universal function - using time_exists_optimization instead of timeless_sparse_tensor_4d_optimization");
		window.setLayout(new BorderLayout());
		final λ wikibinator = null; //FIXME
		EvalPanel panel = new EvalPanel(new Shell(wikibinator));
